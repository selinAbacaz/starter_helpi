...
async function formatQuestionsAndAnswers (userPrompt: string, page: string) { // Function to format the questions and user answers from either the basic/detailed questions
...
async function callGPT (combinedQuestions: string, combinedAnswers: string, userPrompt: string) { // Calls the GPT api and prodcues text based on a user input
  const openai = new OpenAI(
    {
      apiKey: keyData,
      dangerouslyAllowBrowser: true
    });
  const result = await openai.chat.completions.create({ // The generated response by chatGPT
    messages: 
    [
...
export async function GenerateText (type: string, page: string, userInput: string) {
  let result = ""; // Contains the returned message content generated by chatGPT
  if (type === "industry") {
